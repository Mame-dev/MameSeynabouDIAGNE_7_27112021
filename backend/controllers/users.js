...
exports.login = async (req, res, next) => {
    ...
    if (cryptoEmail && req.body.password) {
        ...
        res.status(500).json({
            message: "Entrez votre email et votre mot de passe"
        });
    }
};
...
exports.deleteUser = (req, res, next) => {
    ...
    conn.query(`DELETE FROM user WHERE id = ?`, user_id, (error, result) => {
        if (error) return res.status(400).json({
            error: "Le user n'a pas pu être supprimé"
        });
    ...
    conn.query('SELECT id, username, email FROM user ', (error, result) => {
        if (error) {
            return res
                .status(400)
                .json({
                    error: "impossible d'afficher les listes des membres"
                });
        }
        return res.status(200).json(result);
    });
    ...
    conn.query('SELECT * FROM user WHERE id =?', req.params.id, (error, result) => {
        if (error) {
            return res
                .status(400)
                .json({
                    error: "Impossible d'afficher cet Utilisateur"
                });
        }
        return res.status(200).json(result);
    });
...
exports.modifyUser = (req, res, next) => {
    ...
    if (!email || !password) {
        return res.status(400).json({
            message: "les champs des formulaires ne doivent pas être vide"
        });
    } else {
